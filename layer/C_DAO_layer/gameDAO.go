...
func ReadGamesByUserDAO(id string) []DTO.Game {
	db := database.Db_Connection()

	var game_list []DTO.Game
	db.Table("game").Where("id_user = ?", id).Find(&game_list)
...
func SearchGameDAO(id, key_name string) []DTO.Game {
	db := database.Db_Connection()
	var games []DTO.Game
	// db.Table("game").Where("id_user = ?", id).Where("key_name = ?", key_name).Find(&games)
	db.Table("game").Where(
		db.Where("id_user = ?", id).Where("name LIKE ?", key_name),
	).Find(&games)
	return games
}